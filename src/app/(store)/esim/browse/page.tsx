...
export const metadata: Metadata = {
  ...
  description:
    "Filter and compare eSIM data plans for 190+ countries. Find affordable prepaid travel data with instant activation.",
  alternates: {
    canonical: `${BASE_URL}/esim/browse`,
  },
  openGraph: {
    title: "Browse eSIM Plans | MobiaL",
    description:
      "Filter eSIM plans by country, region, price and data. Instant activation, no roaming fees.",
    url: `${BASE_URL}/esim/browse`,
    images: [
      {
        url: `${BASE_URL}/og-image.png`,
        width: 1200,
        height: 630,
        alt: "MobiaL eSIM Plans — Browse & Filter",
      },
    ],
  },
}
...
function buildPaginationUrl(base: Record<string, string>, page: number): string {
  const params = new URLSearchParams({ ...base, page: String(page) })
  return `/esim/browse?${params.toString()}`
}

export default async function BrowseEsimPage({ searchParams }: PageProps) {
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-6xl">
      {/* Breadcrumb */}
      <nav className="flex items-center gap-1 text-xs text-muted-foreground mb-6 font-medium">
        <Link href="/esim" className="hover:text-foreground transition-colors">
          eSIM Plans
        </Link>
        <ChevronRight className="h-3 w-3" />
        <span className="text-foreground font-bold">Browse All</span>
      </nav>
      ...
      {total > 0 && (
        <p className="text-sm text-muted-foreground font-medium mb-6">
          ...
          {params.country ? ` for ${allCountries.find((c) => c.code === params.country)?.name ?? params.country}` : ""}
          {params.region ? ` in ${regionList.find((r) => r.slug === params.region)?.name ?? params.region}` : ""}
        </p>
      )}
          ...
          {products.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              <p className="text-muted-foreground text-sm max-w-xs">
                ...
                <Link href="/esim/browse" className="text-primary font-bold hover:underline">
                  clear all filters
                </Link>{" "}
